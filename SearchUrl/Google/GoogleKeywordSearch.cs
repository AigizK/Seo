...
namespace SearchUrl.Google
{
    public class GoogleKeywordSearch : IKeywordSearch
    {
            ...
            return new KeywordResult
                {
                    ...
                    Version = "0.0.1",
                    Keyword = keyword,
                    EngineResults = GetEngineResults(keyword)
                };
        }

        private IList<EngineResult> GetEngineResults(string keyword)
        {
            var html = GetHtml(keyword);
            ...
            while ((startIndex = html.Substring(position).IndexOf("<li class=\"g\">")) >= 0)
            {
                ...
                engineResult.ItemHtml = html.Substring(position + startIndex, endIndex);
        ...
        static string GetHtml(string key)
        {
            key = key.Trim();
            string responseData = "";
            try
            {
                string url = string.Format("https://www.google.ru/search?hl=ru&source=hp&biw=&bih=&q={0}&btnG=%D0%9F%D0%BE%D0%B8%D1%81%D0%BA+%D0%B2+Google&gbv=1", key);
